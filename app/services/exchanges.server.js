...
export async function createExchangeRequest(input) {
  ...
  if (!eligibility.eligible) {
    throw new Error(
      `Exchange Policy Rejection (${eligibility.reasonCode}): ${eligibility.message}`,
    );
  }
  ...
  return await prisma.$transaction(async (tx) => {
        ...
        data: input.items.map((item) => {
          const diff =
            item.priceDifference !== undefined
              ...
              : (item.replacementPrice || 0) * item.replacementQuantity -
                (item.originalPrice || 0) * item.originalQuantity;
    ...
    if (!createdExchange) {
      throw new Error(
        `Exchange request ${exchangeRequest.id} could not be reloaded after creation`,
      );
    }
...
export async function getExchangeRequests(shopifyStoreId) {
...
export async function getExchangeRequestById(id, shopifyStoreId) {
...
import { transitionExchangeStatus } from "./stateMachine.server";
