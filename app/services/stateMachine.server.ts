...
import { notifyStatusChange, triggerLifecycleNotification } from "./notifications.server";

export type ReturnStatus = "PENDING" | "APPROVED" | "REJECTED" | "COMPLETED" | "CANCELLED";
export type ExchangeStatus = "PENDING" | "APPROVED" | "REJECTED" | "FULFILLED" | "COMPLETED" | "CANCELLED";
...
const ALLOWED_EXCHANGE_TRANSITIONS: Record<ExchangeStatus, ExchangeStatus[]> = {
  PENDING: ["APPROVED", "REJECTED", "CANCELLED"],
...
export interface TransitionExchangeInput {
  id: string;
  shopifyStoreId: string;
...
export async function transitionReturnStatus({
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!returnRequest) {
      throw new Error(`Return request ${id} not found for store ${shopifyStoreId}`);
    }
    ...
    if (currentStatus === targetStatus) {
      return { updated: returnRequest, previousStatus: currentStatus, idempotent: true };
    }
    ...
    const updated = await tx.returnRequest.update({
      ...
      data: {
        status: targetStatus,
        adminNote: adminNote !== undefined ? adminNote : returnRequest.adminNote,
      },
      include: { items: true },
    });
  ...
  if (!result.idempotent) {
    ...
    if (targetStatus === "APPROVED") {
      try {
        await executeShopifyReturnCreate({ shopifyStoreId, entityId: id, admin });
      } catch (err: any) {
        console.error(`[StateMachine] Return ${id} returnCreate error:`, err?.message || err);
      ...
      try {
        await executeShopifyRefundCreate({ shopifyStoreId, entityId: id, admin });
      } catch (err: any) {
        console.error(`[StateMachine] Return ${id} refundCreate error:`, err?.message || err);
        if (err?.message?.includes("userErrors")) {
          throw err;
        }
        await enqueueJob({
      ...
      try {
        await executeShopifyInventoryAdjust({ shopifyStoreId, entityId: id, admin });
      } catch (err: any) {
        console.error(`[StateMachine] Return ${id} inventoryAdjust error:`, err?.message || err);
        if (err?.message?.includes("userErrors")) {
          throw err;
        }
        await enqueueJob({
    ...
    if (targetStatus === "APPROVED") {
      ...
      await triggerLifecycleNotification({
        ...
        variables: {
          customer_name: result.updated.customerName || "Customer",
          order_number: result.updated.orderNumber,
          refund_amount: result.updated.refundAmount ? Number(result.updated.refundAmount) : 0,
        },
      });
      await triggerLifecycleNotification({
        shopifyStoreId,
...
export async function transitionExchangeStatus({
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!exchangeRequest) {
      throw new Error(`Exchange request ${id} not found for store ${shopifyStoreId}`);
    }

    const currentStatus = exchangeRequest.status as ExchangeStatus;

    // Idempotency check: if already at target status, return existing record
    if (currentStatus === targetStatus) {
      return { updated: exchangeRequest, previousStatus: currentStatus, idempotent: true };
    }
  ...
  if (!result.idempotent) {
    if (targetStatus === "APPROVED") {
      try {
        await executeShopifyDraftOrderCreate({ shopifyStoreId, entityId: id, admin });
      } catch (err: any) {
        console.error(`[StateMachine] Exchange ${id} draftOrderCreate error:`, err?.message || err);
        if (err?.message?.includes("userErrors")) {
          throw err;
        }
        await enqueueJob({
      ...
      try {
        ...
        console.error(`[StateMachine] Forward shipment registration error for Exchange ${id}:`, err?.message || err);
      }
    }
