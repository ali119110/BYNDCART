...
const FROM_ADDRESS = process.env.NOTIFICATIONS_FROM_EMAIL || "BYNDCART <notifications@byndcart.app>";
...
export const DEFAULT_TEMPLATES: Record<NotificationEventType, DefaultTemplateConfig> = {
  RETURN_SUBMITTED: {
  ...
  REPLACEMENT_CREATED: {
    ...
    subject: "Replacement Order {{replacement_order}} Created for Order {{order_number}}",
 ...
 */
export function sanitizeLogBody(body: string): string {
  if (!body) return "";
 ...
 */
export async function triggerLifecycleNotification(input: TriggerNotificationInput) {
  const channel = input.channel || "EMAIL";
  const recipient = channel === "EMAIL" ? input.recipientEmail : input.recipientPhone || input.recipientEmail;
  ...
  try {
    ...
    if (existingLog && (existingLog.status === "SENT" || existingLog.status === "SKIPPED" || existingLog.status === "PENDING")) {
      console.log(`[Notifications] Idempotent skip for ${idempotencyKey} (Status: ${existingLog.status})`);
    ...
    console.error(`[Notifications] Error triggering notification for ${input.eventType}:`, err);
...
export async function processNotificationJob(payload: {
  ...
  const { notificationLogId, shopifyStoreId, eventType, channel, recipient, variables } = payload;
  ...
  try {
    ...
    const defaultConfig = DEFAULT_TEMPLATES[eventType] || DEFAULT_TEMPLATES.RETURN_SUBMITTED;
    const isEnabled = customTemplate ? customTemplate.enabled : true;
    const subjectTemplate = customTemplate ? customTemplate.subject : defaultConfig.subject;
    const bodyTemplate = customTemplate ? customTemplate.body : defaultConfig.body;

    // Check if notification event is disabled by merchant
    if (!isEnabled) {
      console.log(`[Notifications] Event ${eventType} is disabled for store ${shopifyStoreId}. Skipping.`);
    ...
    console.error(`[Notifications] Exception during processNotificationJob:`, err);
    try {
      await prisma.notificationLog.update({
        where: { id: notificationLogId },
        data: {
 ...
 */
export async function getStoreNotificationTemplates(shopifyStoreId: string) {
  ...
  return Object.values(DEFAULT_TEMPLATES).map((def) => {
    const custom = dbTemplates.find((t) => t.eventType === def.eventType && t.channel === "EMAIL");
    return {
...
export async function getNotificationLogs(shopifyStoreId: string, limit = 20) {
 ...
 */
export async function notifyStatusChange(input: any) {
  let settings;
  try {
