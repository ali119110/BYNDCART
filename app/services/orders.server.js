 ...
 */
function mapWebhookOrder(payload) {
  return {
...
async function upsertOrder(shopifyStoreId, mapped, payloadUpdatedAt) {
  ...
  if (existing && payloadUpdatedAt && existing.updatedAt > payloadUpdatedAt) {
    console.log(
      `[Orders Service] Out-of-order webhook detected for ${mapped.shopifyOrderId}. Skipping stale update.`,
    );
 ...
 */
export async function upsertOrderFromWebhook(shopifyStoreId, payload) {
...
export async function cancelOrderFromWebhook(shopifyStoreId, payload) {
  const mapped = mapWebhookOrder(payload);
 ...
 */
export async function syncShopifyOrders({ shopifyStoreId, admin }) {
  if (!admin) {
    console.log(
      `[Orders Service] Skipping sync for ${shopifyStoreId}: no admin client (local dev)`,
    );
...
export async function searchOrSyncOrders({ shopifyStoreId, admin, query }) {
 ...
 */
export async function getStoreOrders(shopifyStoreId, limit = 50, admin) {
  ...
  const returnStatusByOrder = new Map(
    returns.map((r) => [r.shopifyOrderId, r.status]),
  );
  const exchangeStatusByOrder = new Map(
    exchanges.map((e) => [e.shopifyOrderId, e.status]),
  );
