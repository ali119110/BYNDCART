...
export async function upsertCustomerCache(shopifyStoreId, customer) {
...
export async function deleteCustomerFromWebhook(shopifyStoreId, payload) {
...
export async function upsertCustomerFromWebhook(shopifyStoreId, payload) {
  ...
  const totalSpent = parseFloat(
    payload.total_spent ?? payload.totalSpent ?? "0",
  );
...
export async function syncShopifyCustomers(shopifyStoreId, admin) {
...
export async function getStoreCustomers(shopifyStoreId, search = "") {
  ...
  if (cached.length > 0) {
    for (const c of cached) {
      ...
      const customerExchanges = exchanges.filter(
        (e) => e.customerEmail === email,
      );
      ...
      if (retCount >= 3)
        reasons.push(`High return count: ${retCount} returns on file`);
      if (parseFloat(rateVal) > 30)
        reasons.push(`Excessive return rate: ${rateVal}%`);
    ...
    const emailOrderCounts = new Map();
