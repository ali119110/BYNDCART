 ...
 */
export async function handleCustomerDataRequest(shopifyStoreId: string, payload: any) {
  const email = payload?.customer?.email;
  if (!email) {
    console.warn(`[Compliance] customers/data_request missing customer email for store ${shopifyStoreId}`);
    return;
  }
  ...
  console.log(`[Compliance] Logged data request for ${email} on store ${shopifyStoreId}`);
}
 ...
 */
export async function handleCustomerRedact(shopifyStoreId: string, payload: any) {
  const email = payload?.customer?.email;
  if (!email) {
    console.warn(`[Compliance] customers/redact missing customer email for store ${shopifyStoreId}`);
    return;
  }
  ...
  const [returnsResult, exchangesResult] = await Promise.all([
    prisma.returnRequest.updateMany({
      where: { shopifyStoreId, customerEmail: email },
      data: { customerEmail: redactedEmail, customerName: null, customerNote: null, adminNote: null },
    }),
...
export async function handleShopRedact(shop: string) {
  ...
  if (!store) {
    console.warn(`[Compliance] shop/redact: store ${shop} not found (already deleted?)`);
    return;
  }
