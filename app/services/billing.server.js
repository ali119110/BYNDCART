...
export async function createAppSubscription(
  ...
  if (planCode === "FREE") {
    ...
    if (existingSub.shopifySubscriptionId && adminContext) {
      try {
        ...
        console.warn(
          `[Billing Service] Warning during cancel while downgrading:`,
          err,
        );
      }
    }
  ...
  try {
    ...
    console.warn(
      `[Billing Service] GraphQL appSubscriptionCreate execution note: ${err.message}`,
    );
 ...
 */
export async function cancelAppSubscription(shopifyStoreId, adminContext) {
  const sub = await getStoreSubscription(shopifyStoreId);

  if (
    sub.shopifySubscriptionId &&
    sub.shopifySubscriptionId.startsWith("gid://shopify/AppSubscription/")
  ) {
    try {
      const store = await prisma.shopifyStore.findUnique({
        where: { id: shopifyStoreId },
      ...
      console.warn(
        `[Billing Service] Cancel GraphQL execution note: ${err.message}`,
      );
    }
  }
 ...
 */
export async function syncSubscriptionState(shopifyStoreId, adminContext) {
  const store = await prisma.shopifyStore.findUnique({
    where: { id: shopifyStoreId },
  });
  ...
  try {
    ...
    console.warn(
      `[Billing Service] syncSubscriptionState note: ${err.message}`,
    );
  }
 ...
 */
export async function handleSubscriptionWebhook(shop, payload) {
  const store = await prisma.shopifyStore.findUnique({
    where: { shop },
  });
