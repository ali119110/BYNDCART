...
export async function registerStore({ shop, accessToken }) {
  try {
    ...
    try {
      if (accessToken) {
        ...
        if (response.ok) {
          ...
          if (shopData?.contactEmail || shopData?.email)
            email = shopData.contactEmail || shopData.email;
        }
      }
    } catch (err) {
      console.warn(
        `[registerStore] Could not fetch GraphQL details for ${shop}, using default shop name fallback:`,
        err,
      );
    }
...
export async function getStoreByShop(shop) {
