 ...
 */
export async function registerShipmentTrack(input) {
  ...
  let adapter = input.courier
    ...
    : await getDefaultMerchantCourier(
        input.shopifyStoreId,
        shipmentType === "FORWARD" ? "FORWARD" : "REVERSE",
      );
  // Load merchant's decrypted API credentials
  const credentials = await getDecryptedCourierCredentials(
    input.shopifyStoreId,
  ...
  if (!trackingNumber) {
    try {
      ...
      console.error(
        `[Shipment Service] External Courier API failure for ${orderNumber}:`,
        err.message,
      );
      ...
      throw new Error(
        `Courier booking failed for ${adapter.providerName}: ${err.message}. Queued for retry.`,
      );
    }
  }
    ...
    await prisma.shipmentEvent.create({
      data: {
        ...
        description:
          ...
          `Shipment registered via ${adapter.displayName}`,
        location: trackingInfo.location || null,
        rawPayload:
          trackingInfo.rawPayload ||
          (labelResult ? labelResult.rawResponse : null),
      },
    });
  } catch {
    // Offline unit test fallback
 ...
 */
export async function syncShipmentTrackingStatus(trackingNumber) {
  ...
  if (
    !latestEvent ||
    latestEvent.status !== normalizedStatus ||
    latestEvent.description !== latestDesc
  ) {
    await prisma.shipmentEvent.create({
      data: {
        shipmentTrackId: shipment.id,
 ...
 */
export async function getStoreShipments(shopifyStoreId, options = {}) {
  const { type, search, limit = 50 } = options;
  const where = { shopifyStoreId };
