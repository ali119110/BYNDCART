 ...
 */
export async function getShopifyAdminClient(shopifyStoreId, providedAdmin) {
  ...
  try {
    ...
    console.warn(
      `[ShopifySync] Could not resolve offline admin client for store ${shopifyStoreId}:`,
      error,
    );
 ...
 */
export async function executeShopifyReturnCreate({
  shopifyStoreId,
  entityId,
  admin,
}) {
  ...
  if (!returnRequest) {
    throw new Error(
      `Return request ${entityId} not found for store ${shopifyStoreId}`,
    );
  }
  ...
  if (returnRequest.shopifyReturnId) {
    console.log(
      `[ShopifySync] Idempotent skip: returnCreate already executed for Return ${entityId} (${returnRequest.shopifyReturnId})`,
    );
  ...
  if (!client) {
    console.log(
      `[ShopifySync] No admin client available — skipping live returnCreate for Return ${entityId}`,
    );
  ...
  if (userErrors.length > 0) {
    const errorMsg = `Shopify returnCreate userErrors: ${userErrors.map((e) => `${e.field ? e.field.join(".") + ": " : ""}${e.message}`).join("; ")}`;
  ...
  if (!shopifyReturn?.id) {
    throw new Error(
      "Shopify returnCreate returned no return object and no userErrors.",
    );
  }
 ...
 */
export async function executeShopifyRefundCreate({
  shopifyStoreId,
  entityId,
  admin,
}) {
  ...
  if (returnRequest.shopifyRefundId) {
    console.log(
      `[ShopifySync] Idempotent skip: refundCreate already executed for Return ${entityId} (${returnRequest.shopifyRefundId})`,
    );

    return {
      success: true,
      shopifyRefundId: returnRequest.shopifyRefundId,
      idempotent: true,
    };
  }
  ...
  if (!client) {
    console.log(
      `[ShopifySync] No admin client available — skipping live refundCreate for Return ${entityId}`,
    );
  ...
  const refundAmountVal = returnRequest.refundAmount
  ...
  if (userErrors.length > 0) {
    const errorMsg = `Shopify refundCreate userErrors: ${userErrors.map((e) => `${e.field ? e.field.join(".") + ": " : ""}${e.message}`).join("; ")}`;
  ...
  if (!refund?.id) {
    throw new Error(
      "Shopify refundCreate returned no refund object and no userErrors.",
    );
  }
 ...
 */
export async function executeShopifyInventoryAdjust({
  shopifyStoreId,
  entityId,
  admin,
}) {
  ...
  if (priorLog) {
    console.log(
      `[ShopifySync] Idempotent skip: inventoryAdjustQuantities already executed for Return ${entityId}`,
    );
  ...
  if (!client) {
    console.log(
      `[ShopifySync] No admin client available — skipping live inventoryAdjustQuantities for Return ${entityId}`,
    );
  ...
  const locationRes = await client.graphql(`#graphql
    query getLocations {
      locations(first: 1) {
        edges {
          node {
  ...
  if (!locationId) {
    throw new Error(
      "Could not resolve Shopify store primary location ID for inventory adjustment.",
    );
  }
  ...
  if (userErrors.length > 0) {
    const errorMsg = `Shopify inventoryAdjustQuantities userErrors: ${userErrors.map((e) => `${e.field ? e.field.join(".") + ": " : ""}${e.message}`).join("; ")}`;
 ...
 */
export async function executeShopifyDraftOrderCreate({
  shopifyStoreId,
  entityId,
  admin,
}) {
  ...
  if (!exchangeRequest) {
    throw new Error(
      `Exchange request ${entityId} not found for store ${shopifyStoreId}`,
    );
  }
  ...
  if (exchangeRequest.newOrderId) {
    console.log(
      `[ShopifySync] Idempotent skip: draftOrderCreate already executed for Exchange ${entityId} (${exchangeRequest.newOrderId})`,
    );
  ...
  if (!client) {
    console.log(
      `[ShopifySync] No admin client available — skipping live draftOrderCreate for Exchange ${entityId}`,
    );
  ...
  const netPriceDiff = exchangeRequest.items.reduce(
    (acc, item) => acc + Number(item.priceDifference || 0),
    0,
  );
  const diffLabel =
    netPriceDiff > 0
      ? `+PKR ${netPriceDiff} (Customer Balance Owed)`
      : netPriceDiff < 0
        ? `-PKR ${Math.abs(netPriceDiff)} (Store Credit / Partial Refund Owed)`
        : "Even Exchange (0 Balance)";
  const response = await client.graphql(
    `#graphql
      ...
      variables: {
        input: {
          ...
          tags: [
            ...
            `original-order-${exchangeRequest.orderNumber}`,
            `price-diff-${netPriceDiff}`,
          ],
          lineItems,
        },
      },
  ...
  if (userErrors.length > 0) {
    const errorMsg = `Shopify draftOrderCreate userErrors: ${userErrors.map((e) => `${e.field ? e.field.join(".") + ": " : ""}${e.message}`).join("; ")}`;
  ...
  if (!draftOrder?.id) {
    throw new Error(
      "Shopify draftOrderCreate returned no draftOrder object and no userErrors.",
    );
  }
 ...
 */
export async function executeInitialShopifySync({
  shopifyStoreId,
  admin: providedAdmin,
}) {
  ...
  try {
    ...
    const totalSynced =
      (ordersResult.synchronizedCount || 0) +
      (productsResult.synchronizedCount || 0) +
    ...
    await prisma.auditLog.create({
      data: {
        ...
        metadata: {
          totalSynced,
          orders: ordersResult.synchronizedCount,
          products: productsResult.synchronizedCount,
    ...
    console.error(
      `[ShopifySync] Initial sync failed for store ${shopifyStoreId}:`,
      error,
    );
    await prisma.shopifyStore.update({
      where: { id: shopifyStoreId },
 ...
 */
export async function executeWebhookRegistration({
  shopifyStoreId,
  admin: providedAdmin,
}) {
  try {
    ...
    console.error(
      `[WebhookRegistration] Webhook registration failed for store ${shopifyStoreId}:`,
      error,
    );
    await prisma.shopifyStore.update({
      where: { id: shopifyStoreId },
      data: {
        ...
        lastSyncError: `Webhook reg failed: ${errorMsg}`,
      },
    });
    throw error;
  }
 ...
 */
export async function executeShopifyReconciliation({
  shopifyStoreId,
  admin: providedAdmin,
}) {
  try {
    ...
    console.error(
      `[Reconciliation] Failed for store ${shopifyStoreId}:`,
      error,
    );
    throw error;
  }
