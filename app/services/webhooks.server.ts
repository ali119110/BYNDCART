...
export async function ingestWebhook(
  ...
  try {
    ...
    if (existing) {
      console.log(`[Webhook] Duplicate event detected: ${webhookId} (${topic}). Skipping.`);
    ...
    if (!store) {
      console.warn(`[Webhook] Store not found: ${shop}. Skipping event recording.`);
    ...
    try {
      processingSuccess = await routeAndProcessWebhook(store.id, topic, jsonPayload, admin, shop);
    } catch (err) {
      console.error(`[Webhook] Error during in-band processing for ${topic}:`, err);
    }
    ...
    if (processingSuccess) {
      try {
        ...
        console.log(`[Webhook] Could not mark ${webhookId} processed (row likely cascaded away): ${err}`);
      }
    }

    return { duplicate: false, success: true, topic, jobId: job.id };
  } catch (error) {
    console.error(`[Webhook] Fatal error during webhook ingestion for ${webhookId}:`, error);
    return { duplicate: false, success: false, topic };
  }
}
...
async function routeAndProcessWebhook(
  ...
  console.log(`[Webhook] Processing handler for store ${shopifyStoreId}, topic ${topic}`);
