...
// Mock Prisma Methods
prisma.merchant.create = (async (args: any) => {
  const m = {
...
prisma.billingSubscription.findUnique = (async (args: any) => {
  if (args.where.shopifyStoreId) {
    return mockSubscriptions.find((sub) => sub.shopifyStoreId === args.where.shopifyStoreId) || null;
  }
  if (args.where.shopifySubscriptionId) {
    return mockSubscriptions.find((sub) => sub.shopifySubscriptionId === args.where.shopifySubscriptionId) || null;
  }
  return null;
}) as any;

prisma.billingSubscription.create = (async (args: any) => {
  const sub = {
...
prisma.billingSubscription.update = (async (args: any) => {
  const idx = mockSubscriptions.findIndex(
    (sub) => sub.id === args.where.id || sub.shopifyStoreId === args.where.shopifyStoreId
  );
  if (idx >= 0) {
...
prisma.billingSubscription.upsert = (async (args: any) => {
  const storeId = args.where.shopifyStoreId;
  const idx = mockSubscriptions.findIndex((sub) => sub.shopifyStoreId === storeId);
  if (idx >= 0) {
    mockSubscriptions[idx] = {
      ...mockSubscriptions[idx],
      ...args.update,
...
test("Billing & PlanGate — Default Free Plan Creation", async () => {
  const { store } = await createTestStore(`test-billing-free-${Date.now()}.myshopify.com`);
...
test("Billing & PlanGate — New Subscription Initiation", async () => {
  const { store } = await createTestStore(`test-billing-new-${Date.now()}.myshopify.com`);
...
test("Billing & PlanGate — Existing Active Subscription Query", async () => {
  const { store } = await createTestStore(`test-billing-exist-${Date.now()}.myshopify.com`);
...
test("Billing & PlanGate — Subscription Cancellation", async () => {
  const { store } = await createTestStore(`test-billing-cancel-${Date.now()}.myshopify.com`);
...
test("Billing & PlanGate — Duplicate Billing Request Idempotency", async () => {
  const { store } = await createTestStore(`test-billing-idempotent-${Date.now()}.myshopify.com`);
...
test("Billing & PlanGate — Monthly Return Usage Limit Enforcement", async () => {
  const { store } = await createTestStore(`test-billing-limit-${Date.now()}.myshopify.com`);
  ...
  await assert.rejects(
    ...
    (err: any) => {
      return err.code === "PLAN_USAGE_LIMIT_EXCEEDED" || err.message.includes("Plan usage limit reached");
    }
  );
});

test("Billing & PlanGate — Expired Subscription Fallback", async () => {
  const { store } = await createTestStore(`test-billing-expired-${Date.now()}.myshopify.com`);
...
test("Billing & PlanGate — Cross-Merchant Tenant Isolation", async () => {
  const { store: storeA } = await createTestStore(`test-tenant-a-${Date.now()}.myshopify.com`);
  const { store: storeB } = await createTestStore(`test-tenant-b-${Date.now()}.myshopify.com`);
...
test("Billing & PlanGate — Webhook Processing (APP_SUBSCRIPTIONS_UPDATE)", async () => {
  ...
  assert.equal(result.shopifySubscriptionId, "gid://shopify/AppSubscription/wh_776655");
});
