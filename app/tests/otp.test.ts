...
import { lookupShopperOrder, submitShopperReturn } from "../services/portal.server";
...
prisma.customerOTP.updateMany = (async (args: any) => {
...
prisma.customerOTP.create = (async (args: any) => {
  const newRec = {
...
prisma.customerOTP.findFirst = (async (args: any) => {
  const matching = mockOtpStore.filter((rec) => {
    if (args.where.shopifyStoreId && rec.shopifyStoreId !== args.where.shopifyStoreId) return false;
    if (args.where.orderNumber && rec.orderNumber !== args.where.orderNumber) return false;
    if (args.where.destination && rec.destination !== args.where.destination) return false;
    if (args.where.verified !== undefined && rec.verified !== args.where.verified) return false;
    if (args.where.expiresAt?.gt && new Date(rec.expiresAt).getTime() <= new Date(args.where.expiresAt.gt).getTime()) return false;
    return true;
  });
...
prisma.customerOTP.update = (async (args: any) => {
  const rec = mockOtpStore.find((r) => r.id === args.where.id);
  if (rec) {
    if (args.data.verified !== undefined) rec.verified = args.data.verified;
    if (args.data.attempts?.increment) rec.attempts += args.data.attempts.increment;
  }
  return rec;
}) as any;
...
prisma.returnItem.createMany = (async () => ({ count: 1 })) as any;
prisma.shipmentTrack.upsert = (async (args: any) => ({ id: "ship-1", trackingNumber: "777000111" })) as any;
...
test("OTP & Security — Request OTP & Anti-Enumeration", async () => {
  ...
  const resValid = await requestCustomerOTP("store-1", "#1001", "shopper@example.pk");
  ...
  const resInvalid = await requestCustomerOTP("store-1", "#9999", "hacker@example.pk");
...
test("OTP & Security — Valid OTP Verification & Session Generation", async () => {
  ...
  const res = await verifyCustomerOTP("store-1", "#1001", "shopper@example.pk", rawCode);
...
test("OTP & Security — Invalid OTP Rejection & Failed Attempt Counter", async () => {
  ...
  const res = await verifyCustomerOTP("store-1", "#1001", "shopper@example.pk", "999999");
...
test("OTP & Security — Expired OTP Rejection", async () => {
  ...
  const res = await verifyCustomerOTP("store-1", "#1001", "shopper@example.pk", "112233");
  assert.strictEqual(res.success, false);
...
test("OTP & Security — Reused OTP Rejection", async () => {
  ...
  const res = await verifyCustomerOTP("store-1", "#1001", "shopper@example.pk", "445566");
  assert.strictEqual(res.success, false);
  assert.ok(res.error?.includes("invalid or has expired"));
});
...
test("OTP & Security — Maximum Verification Attempts Lockout", async () => {
  ...
  const res = await verifyCustomerOTP("store-1", "#1001", "shopper@example.pk", "888888");
  assert.strictEqual(res.success, false);
...
test("OTP & Security — Session Token Verification & Expiry", async () => {
  ...
  const parsedValid = verifyCustomerSessionToken(validToken, "store-1", "#1001");
  ...
  const parsedExpired = verifyCustomerSessionToken(expiredToken, "store-1", "#1001");
...
test("OTP & Security — Cross-Merchant Access Prevention", async () => {
  ...
  const parsedCross = verifyCustomerSessionToken(store1Token, "store-2", "#1001");
...
test("OTP & Security — Changing Order Number After Authentication Prevention", async () => {
  ...
  const parsedTampered = verifyCustomerSessionToken(tokenForOrder1001, "store-1", "#9999");
