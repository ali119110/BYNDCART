...
test("State Machine - Return status transitions & graph enforcement", async () => {
  const { transitionReturnStatus } =
    await import("../services/stateMachine.server");
  ...
  await assert.rejects(
    ...
    {
      message:
        /Invalid return status transition: cannot move from 'PENDING' to 'COMPLETED'/,
    },
  );
});
test("State Machine - Exchange status transitions", async () => {
  const { transitionExchangeStatus } =
    await import("../services/stateMachine.server");
