...
import { transitionExchangeStatus } from "../services/stateMachine.server";
import { executeShopifyDraftOrderCreate } from "../services/shopifySync.server";
import { registerShipmentTrack } from "../services/shipments.server";
...
let mockExchangeItems: any[] = [];
let mockShipments: any[] = [];
let mockAuditLogs: any[] = [];
let mockJobs: any[] = [];
let mockLogs: any[] = [];

(prisma as any).merchant = {
  create: async (args: any) => {
    const m = { id: `merch-${Date.now()}-${Math.random()}`, ...args.data, createdAt: new Date() };
    mockMerchants.push(m);
    return m;
  },
};

(prisma as any).shopifyStore = {
  create: async (args: any) => {
    const s = { id: `store-${Date.now()}-${Math.random()}`, ...args.data, createdAt: new Date() };
    mockStores.push(s);
    return s;
  },
  findUnique: async (args: any) => {
    if (args.where.id) return mockStores.find((s) => s.id === args.where.id) || null;
    if (args.where.shop) return mockStores.find((s) => s.shop === args.where.shop) || null;
    return null;
  },
};
...
(prisma as any).billingSubscription = {
  findUnique: async (args: any) => {
    return mockSubscriptions.find((s) => s.shopifyStoreId === args?.where?.shopifyStoreId) || {
      id: "sub-1",
...
(prisma as any).returnPolicyRule = {
  findUnique: async (args: any) => {
...
(prisma as any).order = {
  findFirst: async (args: any) => {
    ...
    return (
      mockOrders.find(
        (o) => o.shopifyStoreId === storeId && o.shopifyOrderId === args.where.shopifyOrderId
      ) || {
        id: "ord-101",
...
(prisma as any).exchangeRequest = {
  ...
  findFirst: async (args: any) => {
    ...
    const storeId = args?.where?.shopifyStoreId;
    const req = mockExchangeRequests.find((e) => e.id === id && (!storeId || e.shopifyStoreId === storeId));
    if (!req) return null;
    return {
      ...req,
  ...
  update: async (args: any) => {
    ...
    if (idx >= 0) {
      mockExchangeRequests[idx] = { ...mockExchangeRequests[idx], ...args.data, updatedAt: new Date() };
      return {
        ...mockExchangeRequests[idx],
        items: mockExchangeItems.filter((i) => i.exchangeRequestId === args.where.id),
      };
    }
...
(prisma as any).shipmentTrack = {
  findFirst: async (args: any) => {
    if (args?.where?.exchangeRequestId) {
      return mockShipments.find((s) => s.exchangeRequestId === args.where.exchangeRequestId) || null;
    }
    return null;
  },
  findUnique: async (args: any) => {
    return mockShipments.find((s) => s.trackingNumber === args.where.trackingNumber) || null;
  },
...
(prisma as any).shipmentEvent = {
  create: async (args: any) => ({ id: `event-${Date.now()}`, ...args.data }),
};
...
(prisma as any).backgroundJob = {
  create: async (args: any) => {
    const job = { id: `job-${Date.now()}`, ...args.data, createdAt: new Date() };
    mockJobs.push(job);
    return job;
  },
};
...
(prisma as any).notificationLog = {
  ...
  create: async (args: any) => ({ id: `nlog-${Date.now()}`, ...args.data }),
  upsert: async (args: any) => ({ id: `nlog-${Date.now()}`, ...args.create }),
  update: async () => ({}),
};
...
(prisma as any).merchantCourierConfig = {
  findUnique: async () => null,
  findMany: async () => [],
};
...
async function createTestStore(shopName: string) {
...
test("Exchange Lifecycle — Request Creation with Policy Check & PlanGate", async () => {
  const { store } = await createTestStore(`ex-create-${Date.now()}.myshopify.com`);
...
test("Exchange Lifecycle — Duplicate Replacement Order Idempotency Prevention", async () => {
  const { store } = await createTestStore(`ex-idem-${Date.now()}.myshopify.com`);
  ...
  await prisma.exchangeRequest.update({
    ...
    data: { newOrderId: "gid://shopify/DraftOrder/888", newOrderNumber: "#DRAFT-888" },
  });
...
test("Exchange Lifecycle — Price Difference Calculations (Same, Higher, Lower Price)", async () => {
  const { store } = await createTestStore(`ex-pricediff-${Date.now()}.myshopify.com`);
...
test("Exchange Lifecycle — Premature Completion Rejection", async () => {
  const { store } = await createTestStore(`ex-premature-${Date.now()}.myshopify.com`);
...
test("Exchange Lifecycle — Outbound Forward Courier Shipment Booking", async () => {
  const { store } = await createTestStore(`ex-forward-ship-${Date.now()}.myshopify.com`);
  ...
  const approved = await updateExchangeStatus(exchange!.id, store.id, "APPROVED");
...
test("Exchange Lifecycle — Replacement Delivery & Final Exchange Completion", async () => {
  const { store } = await createTestStore(`ex-complete-${Date.now()}.myshopify.com`);
  ...
  const completed = await updateExchangeStatus(exchange!.id, store.id, "COMPLETED");

  assert.equal(completed.status, "COMPLETED");

  const auditLog = mockAuditLogs.find(
    (a) => a.entityId === exchange!.id && a.action === "EXCHANGE_STATUS_TRANSITION_COMPLETED"
  );
  assert.ok(auditLog);
});

test("Exchange Lifecycle — Cross-Merchant Tenant Isolation", async () => {
  const { store: storeA } = await createTestStore(`ex-tenant-a-${Date.now()}.myshopify.com`);
  const { store: storeB } = await createTestStore(`ex-tenant-b-${Date.now()}.myshopify.com`);
  ...
  await assert.rejects(
    ...
    (err: any) => err.message.includes("not found for store")
  );
});
