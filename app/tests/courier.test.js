...
prisma.merchantCourierConfig.findUnique = async (args) => {
  ...
  return (
    mockConfigs.find(
      (c) => c.shopifyStoreId === storeId && c.providerName === provider,
    ) || null
  );
};

prisma.merchantCourierConfig.findMany = async (args) => {
  return mockConfigs.filter(
    (c) => c.shopifyStoreId === args.where?.shopifyStoreId,
  );
};

prisma.merchantCourierConfig.findFirst = async (args) => {
  return (
    mockConfigs.find(
      (c) =>
...
prisma.merchantCourierConfig.updateMany = async (args) => {
  mockConfigs.forEach((c) => {
    if (c.shopifyStoreId === args.where.shopifyStoreId) {
      if (args.data.isDefaultReverse !== undefined)
        c.isDefaultReverse = args.data.isDefaultReverse;
      if (args.data.isDefaultForward !== undefined)
        c.isDefaultForward = args.data.isDefaultForward;
    }
  });
...
prisma.merchantCourierConfig.upsert = async (args) => {
  const storeId = args.where.shopifyStoreId_providerName.shopifyStoreId;
  const provider = args.where.shopifyStoreId_providerName.providerName;
  const idx = mockConfigs.findIndex(
    (c) => c.shopifyStoreId === storeId && c.providerName === provider,
  );
...
prisma.shipmentTrack.update = async (args) => {
  const idx = mockShipments.findIndex(
    (s) => s.trackingNumber === args.where.trackingNumber,
  );
...
prisma.shipmentTrack.findMany = async (args) => {
  const matches = mockShipments.filter(
    (s) => s.shopifyStoreId === args.where?.shopifyStoreId,
  );

  return matches.map((m) => ({
    ...m,
    events: mockEvents.filter((e) => e.shipmentTrackId === m.id),
  }));
};

prisma.shipmentEvent.create = async (args) => {
...
test("Multi-Courier Framework — Status Normalization", async () => {
  ...
  assert.strictEqual(
    normalizeCourierStatus("Out for delivery to customer"),
    "OUT_FOR_DELIVERY",
  );
  assert.strictEqual(
    normalizeCourierStatus("Delivered successfully"),
    "DELIVERED",
  );
  assert.strictEqual(
    normalizeCourierStatus("Cancelled by merchant"),
    "CANCELLED",
  );
  assert.strictEqual(
    normalizeCourierStatus("Return Failed / Rejected"),
    "FAILED",
  );
  assert.strictEqual(
    normalizeCourierStatus("UNRECOGNIZED_STATUS_XYZ"),
    "UNKNOWN",
  );
});
