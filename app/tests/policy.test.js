...
prisma.returnPolicyRule.findUnique = async (args) => {
  return (
...
prisma.returnPolicyRule.upsert = async (args) => {
...
prisma.returnPolicyRule.create = async (args) => {
  const newRule = {
    id: `rule-uuid-${Date.now()}-${Math.random()}`,
    ...args.data,
    createdAt: new Date(),
...
prisma.returnRequest.count = async (args) => {
  ...
  return mockReturnRequests.filter(
    (r) => !storeId || r.shopifyStoreId === storeId,
  ).length;
};
...
prisma.shipmentTrack.upsert = async () => ({ trackingNumber: "77000111" });
