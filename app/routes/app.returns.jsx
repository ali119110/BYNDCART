...
export async function loader({ request }) {
  ...
  try {
    ...
    return { returns: [], orders: [], riskFlags: {} };
  }
}
...
export async function action({ request }) {
  ...
  if (actionType === "CREATE_MANUAL_RETURN") {
    ...
    if (!shopifyOrderId || !orderNumber || !itemsJson) {
      return {
        error: "Order and items selection are required for manual return.",
      };
    }
    ...
    if (!existingOrder) {
      return {
        error:
          "Invalid order selection. Order does not belong to this merchant store.",
      };
    }
    ...
    try {
      ...
      return {
        success: true,
        message: `Manual return for ${orderNumber} created successfully!`,
...
export default function Returns() {
  ...
  const availableOrders = orderSearchFetcher.data?.orders ?? dbOrders;
  // Manual Return Creation Modal State
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [orderSearchQuery, setOrderSearchQuery] = useState("");
  const [selectedOrderId, setSelectedOrderId] = useState("");
  const [selectedItems, setSelectedItems] = useState({});
  const [refundAmountInput, setRefundAmountInput] = useState("0.00");
  const [returnReasonInput, setReturnReasonInput] = useState("DEFECTIVE");
  ...
  const handleCreateManualReturnSubmit = () => {
    ...
    const itemsArray = Object.entries(selectedItems).map(
      ([lineItemId, data]) => ({
        lineItemId,
  ...
  const returnsList = dbReturns.map((ret) => ({
    id: ret.id,
  ...
  const handleUpdateStatus = (id, newStatus) => {
    fetcher.submit({ returnId: id, status: newStatus }, { method: "POST" });
  ...
  const isSubmitting =
    fetcher.state !== "idle" &&
    fetcher.formData?.get("actionType") === "CREATE_MANUAL_RETURN";
    ...
    >
      ...
      >
        <div>
          ...
          >
            Manage customer returns and create manual return requests with full
            order validation.
          </p>
        </div>
        <button
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              />
                              <label
                                htmlFor={`ret-item-${li.lineItemId}`}
                              ...
                              >
                                ...
                                >
                                  Unit Price: PKR {Number(li.price).toFixed(2)}{" "}
                                  | Max Qty: {li.quantity}
                                </div>
                              </label>
                            </div>
              ...
              >
                <s-box padding="base" background="subdued">
                  <h4 style={{ margin: "0 0 8px 0" }}>
                    Confirm Manual Return Creation
                  </h4>
                  <p
                    style={{ margin: 0, fontSize: "13.5px", color: "#475569" }}
                  >
                    Please confirm creating return request for{" "}
                    <strong>{selectedOrder?.orderNumber}</strong> with{" "}
                    <strong>{Object.keys(selectedItems).length} item(s)</strong>{" "}
                    and a refund amount of{" "}
