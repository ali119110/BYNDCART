...
export const action = async ({ request }) => {
  ...
  if (actionType === "subscribe") {
    ...
    try {
      ...
      return {
        success: true,
        message: result.message || `Subscribed to ${planCode}`,
      };
    } catch (err) {
...
export default function Billing() {
  const { storePlan: rawStorePlan, plans } = useLoaderData();
  ...
  const usagePercent = storePlan.isUnlimited
    ...
    : Math.min(
        100,
        Math.round(
          ((storePlan.usageCount ?? 0) /
            Math.max(1, storePlan.returnsLimit ?? 15)) *
            100,
        ),
      );
      ...
      <s-stack direction="block" gap="large">
        ...
        >
          <s-stack direction="block" gap="base">
            ...
            >
              <div>
                <s-heading>
                  Current Subscription: {storePlan.name ?? "Free Plan"}
                </s-heading>
                <p style={{ margin: "4px 0 0 0", color: "#475569" }}>
                  ...
                  <strong
                    style={{
                      color:
                        storePlan.status === "ACTIVE" ? "#008000" : "#d97706",
                    }}
                  >
                    {storePlan.status ?? "ACTIVE"}
                  </strong>{" "}
                  — ${(storePlan.price ?? 0).toFixed(2)} / month
                </p>
              </div>
            ...
            >
              ...
              >
                ...
                <span>
                  <strong>{storePlan.usageCount}</strong> /{" "}
                  {storePlan.isUnlimited
                    ? "Unlimited"
                    : `${storePlan.returnsLimit} returns`}
                </span>
              </div>
              {!storePlan.isUnlimited && (
                <div
        ...
        <s-box padding="base">
          ...
          <s-paragraph tone="neutral">
            Choose the best plan for your store's return volume and reverse
                ...
                >
                  ...
                  <fetcher.Form method="post" style={{ marginTop: "16px" }}>
                    ...
                    <s-button
                      ...
                      loading={
                        isSubmitting &&
                        fetcher.formData?.get("planCode") === plan.code
                      }
                    >
                      {isCurrent
                        ? "Current Plan"
