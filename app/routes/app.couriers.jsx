...
export const action = async ({ request }) => {
  ...
  if (intent === "SAVE_CONFIG") {
    ...
    try {
      ...
      return {
        success: true,
        message: `Configuration saved for ${providerName}`,
      };
    } catch (err) {
      return {
        success: false,
  ...
  if (intent === "TOGGLE_ENABLED") {
    ...
    try {
      ...
      return {
        success: true,
        message: `${providerName} ${enabled ? "enabled" : "disabled"}`,
      };
    } catch (err) {
      return {
        success: false,
  ...
  if (intent === "SET_DEFAULT_REVERSE") {
    try {
      ...
      return {
        success: true,
        message: `${providerName} set as default reverse logistics courier`,
      };
    } catch (err) {
  ...
  if (intent === "SET_DEFAULT_FORWARD") {
    try {
      ...
      return {
        success: true,
        message: `${providerName} set as default forward logistics courier`,
      };
    } catch (err) {
      return { success: false, error: err.message || "Failed to set default" };
    }
...
export default function Couriers() {
  ...
  const configMap = new Map(
    (configuredStoreCouriers || []).map((c) => [c.providerName, c]),
  );

  const handleEditClick = (adapter) => {
  ...
  const handleTestConnection = (providerName) => {
    fetcher.submit(
      {
  ...
  const handleSetDefaultReverse = (providerName) => {
    fetcher.submit(
      { intent: "SET_DEFAULT_REVERSE", providerName },
      { method: "post" },
    );
  };

  const handleSetDefaultForward = (providerName) => {
    fetcher.submit(
      { intent: "SET_DEFAULT_FORWARD", providerName },
      { method: "post" },
    );
  };
    ...
    <s-page heading="Pakistani Logistics & Courier Manager">
      ...
      >
        <s-stack direction="block" gap="base">
          ...
          <s-paragraph>
            Configure your merchant API credentials for Pakistan&apos;s leading
            logistics providers. Manage separate default couriers for reverse
            pickup returns and forward exchange deliveries.
          </s-paragraph>
            ...
            {availableAdapters.map((adapter) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {isDefaultRev && (
                      <s-badge tone="info">Default Reverse (Returns)</s-badge>
                    )}
                    {isDefaultFwd && (
                      <s-badge tone="info">Default Forward (Exchanges)</s-badge>
                    )}
                    {config?.maskedToken && (
                  ...
                  {isEditing ? (
                    ...
                    >
                      <h4 style={{ margin: "0 0 10px 0", fontSize: "14px" }}>
                        Configure Credentials ({adapter.providerName})
                      </h4>
                      ...
                      <div style={{ display: "flex", gap: "8px" }}>
                        ...
                        <button
                          onClick={() =>
                            handleTestConnection(adapter.providerName)
                          }
                          disabled={isSubmitting}
                          style={{
                            padding: "6px 12px",
                    ...
                    >
                      ...
                      {!isDefaultRev && (
                        <button
                          onClick={() =>
                            handleSetDefaultReverse(adapter.providerName)
                          }
                          style={{
                            padding: "6px 10px",
                            backgroundColor: "#f4f9f7",
                      ...
                      {!isDefaultFwd && (
                        <button
                          onClick={() =>
                            handleSetDefaultForward(adapter.providerName)
                          }
                          style={{
                            padding: "6px 10px",
