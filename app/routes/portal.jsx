...
export const action = async ({ request }) => {
  ...
  if (intent === "REQUEST_OTP") {
    ...
    try {
      const otpReqResult = await requestCustomerOTP(
        shopifyStoreId,
        orderNumber,
        customerEmail,
      );
  ...
  if (intent === "VERIFY_OTP") {
    ...
    try {
      ...
      const lookupResult = await lookupShopperOrder(
        shopifyStoreId,
  ...
  if (intent === "SUBMIT_RETURN") {
    ...
    try {
      ...
      return {
        success: false,
        error: err.message || "Return submission failed",
      };
    }
  }
...
export default function ShopperPortal() {
  const { availableCouriers } = useLoaderData();
  ...
  const currentStep =
    actionData?.step || (sessionToken ? "AUTHENTICATED" : "REQUEST");
  const lookupData = actionData?.lookupResult;
  ...
  const handleItemQuantityChange = (lineItemId, qty) => {
    setSelectedItems((prev) => {
  ...
  const handleItemReasonChange = (lineItemId, reason) => {
    setSelectedItems((prev) => {
      if (!prev[lineItemId]) return prev;
  ...
  if (lookupData?.order?.lineItems) {
    for (const item of lookupData.order.lineItems) {
      if (selectedItems[item.lineItemId]) {
        totalSelectedPKR +=
          item.price * selectedItems[item.lineItemId].quantity;
      }
    }
  }
  ...
  const restockingFeePKR = Math.round(
    (totalSelectedPKR * restockingFeePercent) / 100,
  );
  const estimatedPickupChargePKR = 200;
  const netRefundPKR = Math.max(
    0,
    totalSelectedPKR - restockingFeePKR - estimatedPickupChargePKR,
  );
  ...
  const handleSubmitReturn = () => {
    ...
    const payloadItems = Object.entries(selectedItems).map(
      ([lineItemId, data]) => {
        const original = lookupData.order.lineItems.find(
  ...
  return (
    ...
    >
      <div style={{ textAlign: "center", marginBottom: "32px" }}>
        ...
        >
          Pakistani Shopper Return Portal 🇵🇰
        </h1>
        <p style={{ color: "#5c5f62", margin: 0 }}>
          Secure 2-Factor verified self-service return & exchange booking across
          Pakistan
        </p>
      </div>
      ...
      {returnResult ? (
        ...
        >
          <h2 style={{ color: "#008060", margin: "0 0 12px 0" }}>
            Return Booking Confirmed! 🎉
          </h2>
          <p style={{ fontSize: "16px", margin: "0 0 24px 0" }}>
            Your return request for order{" "}
            <strong>{returnResult.orderNumber}</strong> has been successfully
            booked.
          </p>
          ...
          >
            <div style={{ marginBottom: "8px" }}>
              <strong>Return Reference ID:</strong>{" "}
            ...
            <div style={{ marginBottom: "8px" }}>
              <strong>Consignment Tracking CN:</strong>{" "}
              <span style={{ fontFamily: "monospace", color: "#2c6ecb" }}>
            ...
            >
              <strong>Estimated Net Refund (PKR):</strong>{" "}
              <span
                style={{
                  fontSize: "18px",
                  color: "#008060",
              ...
              >
                Rs. {returnResult.summaryPKR?.netRefundAmount?.toLocaleString()}
              </span>
            </div>
          </div>
        ...
        <>
          ...
          {currentStep === "REQUEST" && !actionData?.step && (
            ...
            >
              ...
              >
                Enter your order number and email/phone to receive a 6-digit
                verification code.
              </p>
          ...
          {currentStep === "OTP_SENT" && (
            ...
            >
              ...
              >
                ...
                <strong>{actionData?.customerEmail || emailInput}</strong> for
                order{" "}
                <strong>{actionData?.orderNumber || orderNumberInput}</strong>.
              </p>
              ...
              <fetcher.Form method="post">
                ...
                <input
                  type="hidden"
                  name="orderNumber"
                  value={actionData?.orderNumber || orderNumberInput}
                />
                <input
                  type="hidden"
                  name="customerEmail"
                  value={actionData?.customerEmail || emailInput}
                />
          ...
          {currentStep === "AUTHENTICATED" && lookupData && (
            <div>
              ...
              {!lookupData.eligible && (
                ...
                >
                  <strong>Ineligible for Standard Online Return:</strong>{" "}
                  {lookupData.ineligibilityReason}
                </div>
              )}
              ...
              >
                ...
                <h4 style={{ margin: "16px 0 12px 0", fontSize: "15px" }}>
                    ...
                    >
                      ...
                      {isSelected && (
                        ...
                        >
                          <select
                            value={
                              selectedItems[item.lineItemId]?.reason ||
                              "Size Mismatch"
                            }
                            onChange={(e) =>
                          ...
                          >
                            ...
                            <option value="Fabric / Color Difference">
                ...
                {selectedItemsCount > 0 && (
                  ...
                  >
                    <h4 style={{ margin: "0 0 12px 0", fontSize: "15px" }}>
                      Select Doorstep Pickup Courier (Pakistan):
                    </h4>
                    <select
              ...
              {selectedItemsCount > 0 && (
                ...
                >
                  ...
                  >
                    ...
                    <span>
                      - Rs. {estimatedPickupChargePKR.toLocaleString()}
                    </span>
                  </div>
                  ...
                  >
                    ...
                    <span style={{ color: "#008060" }}>
                      Rs. {netRefundPKR.toLocaleString()}
                    </span>
                  </div>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      marginTop: "20px",
                      padding: "14px",
                      backgroundColor: lookupData.eligible
                        ? "#008060"
                        : "#8c9196",
                      color: "#fff",
                      border: "none",
                  ...
                  >
                    {isSubmitting
                      ...
                      : `Confirm Return Request (Rs. ${netRefundPKR.toLocaleString()})`}
                  </button>
                </div>
              )}
            </div>
