          ...
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #e1e3e5" }}>
                ...
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Items Purchased</s-text></th>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Order Value</s-text></th>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Return Status</s-text></th>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Exchange Status</s-text></th>
              ...
              {filteredOrders.map((order: any) => (
                <tr key={order.id} style={{ borderBottom: "1px solid #f1f2f3" }}>
                  ...
                  <td style={{ padding: "12px 8px" }}>
                    ...
                    <div style={{ fontSize: "12px", color: "#6d7175" }}>{order.customerEmail ?? ""}</div>
                  </td>
                  <td style={{ padding: "12px 8px" }}>
                    {(order.lineItems as any[]).map((li) => `${li.title} (x${li.quantity})`).join(", ")}
                  </td>
                  <td style={{ padding: "12px 8px" }}>
                    ${order.totalPrice.toFixed(2)}
                  </td>
                  <td style={{ padding: "12px 8px" }}>
                    <s-text tone={order.returnStatus === "None" ? "neutral" : "warning"}>
                      {order.returnStatus}
                    </s-text>
                  </td>
                  <td style={{ padding: "12px 8px" }}>
                    <s-text tone={order.exchangeStatus === "None" ? "neutral" : "warning"}>
