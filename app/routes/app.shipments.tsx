...
import { getStoreShipments, registerShipmentTrack } from "../services/shipments.server";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const orderNumber = (formData.get("orderNumber") as string) || `#${Math.floor(1000 + Math.random() * 9000)}`;
  const courier = (formData.get("courier") as string) || "STANDARD";
  const type = (formData.get("type") as "RETURN" | "FORWARD" | "EXCHANGE") || "RETURN";
...
export default function Shipments() {
  const { shipments, search, type, availableCouriers } = useLoaderData<typeof loader>();
      ...
      <div style={{ marginBottom: "16px" }}>
        <s-box padding="base" borderWidth="base" borderRadius="base" background="subdued">
          <s-stack direction="block" gap="base">
            ...
            <fetcher.Form method="post" style={{ display: "flex", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
              <input
                type="text"
                name="orderNumber"
                placeholder="Order Number (e.g. #1005)"
                required
                style={{ padding: "8px 12px", border: "1px solid #c9cccf", borderRadius: "4px", fontSize: "14px" }}
              />
              <select
                name="courier"
                style={{ padding: "8px 12px", border: "1px solid #c9cccf", borderRadius: "4px", fontSize: "14px" }}
        ...
        {shipments.length === 0 ? (
          <s-stack direction="block" gap="base">
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 0", width: "100%" }}>
              ...
              <s-paragraph tone="neutral">Generate a return label above or clear filters.</s-paragraph>
            </div>
          </s-stack>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #e1e3e5" }}>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Shipment ID</s-text></th>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Order</s-text></th>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Type</s-text></th>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Courier</s-text></th>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Tracking Number</s-text></th>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Status</s-text></th>
                <th style={{ padding: "12px 8px" }}><s-text tone="neutral">Action</s-text></th>
              </tr>
            </thead>
            <tbody>
              {shipments.map((ship) => (
                <tr key={ship.id} style={{ borderBottom: "1px solid #f1f2f3" }}>
                  <td style={{ padding: "12px 8px" }}><strong>{ship.id.slice(0, 8)}...</strong></td>
                  <td style={{ padding: "12px 8px" }}>{ship.orderNumber}</td>
                  <td style={{ padding: "12px 8px" }}>
                    <s-badge tone={ship.type === "FORWARD" ? "info" : ship.type === "RETURN" ? "critical" : "success"}>
                      {ship.type}
                    </s-badge>
                  </td>
                  <td style={{ padding: "12px 8px" }}><strong>{ship.courier}</strong></td>
                  <td style={{ padding: "12px 8px" }}>
                    <span style={{ fontFamily: "monospace" }}>{ship.trackingNumber}</span>
                  </td>
                  <td style={{ padding: "12px 8px" }}>
                    <s-badge tone={ship.status === "DELIVERED" ? "success" : "warning"}>
                      {ship.status}
                    </s-badge>
                  </td>
                  <td style={{ padding: "12px 8px" }}>
                    {ship.labelUrl ? (
                      <a href={ship.labelUrl} target="_blank" rel="noreferrer" style={{ textDecoration: "none", color: "#005bd3" }}>
                        Download Label
                      </a>
                    ) : (
                      "-"
