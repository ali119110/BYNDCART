...
export async function loader({ request }) {
  try {
    ...
    if (
      store &&
      (store.syncStatus === "PENDING" || !store.lastSyncAt) &&
      admin
    ) {
      try {
    ...
    const revenueRetained = exchangeItems.reduce((acc, item) => {
      const val = item.priceDifference
        ? parseFloat(item.priceDifference.toString())
        : 0;
                    ...
                    <td style={{ padding: "10px 0" }}>
                      <span
                        style={{
                          ...
                          borderRadius: "12px",
                          backgroundColor:
                            e.status === "PENDING" ? "#fef3c7" : "#dcfce7",
