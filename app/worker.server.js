...
export async function ensureReconciliationScheduled(shopifyStoreId) {
...
export async function ensureReconciliationScheduledForAllStores() {
  try {
    ...
    console.error(
      "[Worker Daemon] Error checking store reconciliation schedules:",
      err,
    );
  }
}
...
registerJobHandler("WEBHOOK_PROCESS", async (payload, shopifyStoreId) => {
  const { topic, shop, webhookPayload } = payload;

  console.log(
    `[Worker] Executing WEBHOOK_PROCESS for topic: ${topic}, store: ${shopifyStoreId || shop}`,
  );
...
registerJobHandler("INITIAL_SHOPIFY_SYNC", async (payload, shopifyStoreId) => {
  if (!shopifyStoreId)
    throw new Error("Missing shopifyStoreId for INITIAL_SHOPIFY_SYNC");
...
registerJobHandler(
  ...
  async (payload, shopifyStoreId) => {
    if (!shopifyStoreId)
      throw new Error("Missing shopifyStoreId for REGISTER_SHOPIFY_WEBHOOKS");
...
registerJobHandler(
  ...
  async (payload, shopifyStoreId) => {
    if (!shopifyStoreId)
      throw new Error("Missing shopifyStoreId for SHOPIFY_RECONCILIATION");
    await executeShopifyReconciliation({ shopifyStoreId });
    await ensureReconciliationScheduled(shopifyStoreId);
  },
);
registerJobHandler("SHOPIFY_RETURN_CREATE", async (payload, shopifyStoreId) => {
  ...
  await executeShopifyReturnCreate({
    shopifyStoreId,
    entityId: payload.returnRequestId,
  });
});
registerJobHandler("SHOPIFY_REFUND_CREATE", async (payload, shopifyStoreId) => {
  if (!shopifyStoreId || !payload.returnRequestId) {
...
registerJobHandler(
  ...
  async (payload, shopifyStoreId) => {
    if (!shopifyStoreId || !payload.returnRequestId) {
      throw new Error(
        "Invalid payload for SHOPIFY_INVENTORY_ADJUST background job",
      );
    }
...
registerJobHandler(
  ...
  async (payload, shopifyStoreId) => {
    if (!shopifyStoreId || !payload.exchangeRequestId) {
      throw new Error(
        "Invalid payload for SHOPIFY_DRAFT_ORDER_CREATE background job",
      );
    }

    await executeShopifyDraftOrderCreate({
      shopifyStoreId,
      entityId: payload.exchangeRequestId,
    });
  },
);
registerJobHandler("SEND_NOTIFICATION", async (payload) => {
...
export async function processNextJob() {
  ...
  if (!job) return false;
  console.log(
    `[Worker] Claimed job ${job.id} (type: ${job.type}, attempt: ${job.attempts}/${job.maxAttempts})`,
  );
 ...
 */
export function startWorkerDaemon(pollIntervalMs = 2000) {
  console.log(
    `[Worker Daemon] Starting worker daemon with polling interval ${pollIntervalMs}ms`,
  );
  let isRunning = true;
